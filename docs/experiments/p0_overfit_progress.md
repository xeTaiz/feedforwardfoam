...
| H4 | Same as H1 | Geometry-aware | Fixed high density only for source-foreground cells | running |
...
- 2026-08-09: First O0 invocation exposed upstream Blender's `eval: false` requirement for `transforms_all.json`; switched oracle config to `eval: true` to use native train/test metadata.
