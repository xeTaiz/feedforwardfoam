...
The 20.7 dB plateau triggered a stricter renderer-contract audit. H4 was not an identity initialization: the head supplied `[0,1]` sigmoid RGB although upstream spherical-Voronoi values are centered and receive `+0.5`; it supplied positive physical radii although `PowerfoamScene.get_radii()` applies another `softplus(beta=100)`; density 100 is only finite Beer–Lambert attenuation; and the identity quaternion produces a world-`+X` dipole normal rather than a camera-facing normal. Therefore H4 cannot establish whether exact pixel-ray initialization reproduces the image.

`scripts/check_same_view_identity.py` bypasses VGGT and the decoder and constructs one cell per exact renderer ray with centered source RGB, inverse-softplus physical radius, camera-facing normals, and effectively opaque density. On Lego frame 0 at 160×160, depth 2.0, radius scale 0.8, and density 10,000, it rendered **154.92 dB PSNR** (MSE `3.22e-16`, MAE `6.52e-9`, max error `2.38e-7`, alpha min/mean `1.0`, radius round-trip error `1.40e-9`). This confirms the renderer and camera path can reproduce the same image essentially exactly. A second direct-color optimization from gray reached **35.82 dB** in 200 Adam steps; it is a useful gradient check, but the remaining plateau is optimizer parameterization rather than representational capacity because direct source initialization already proves the exact solution exists.

## Residual-head correction and ladder

`CanonicalPowerFoamHead` now predicts bounded residuals around a physically valid base: depth-ray points, finite-difference depth normals oriented toward the camera, physical pixel-footprint radii converted through inverse `softplus(beta=100)`, centered source RGB, and raw upstream density. H6 runs the actual head with constant depth and all residual scales zero; it achieves the metric-capped **100 dB** (MSE `4.61e-16`, alpha `1.0`), confirming the head tensor contract is now exact. H8 uses actual frozen VGGT-Ω depth, depth normals, source RGB, and all residual branches; it reaches **32.91 dB**. The remaining error correlates with only `0.916` mean alpha: depth discontinuities leave gaps even though each source ray owns a cell. A radius sweep over 1.0/1.25/1.5 pixel pitches only raises alpha to `0.918/0.922/0.923`, so a larger radius alone is not the right fix; depth-edge coverage needs an explicit edge-aware footprint/dilation or layered cells. H7 fixes exact geometry but initializes color at gray and learns it through the shared decoder; after correcting the centered-color range and optimizer, it reaches **35.88 dB at 1,000 steps**. This confirms decoder gradients and one-image memorization, while showing that a convolutional/shared decoder is intentionally not equivalent to 25,600 independent per-cell colors.
